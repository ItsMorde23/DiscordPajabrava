...
const DeafenOffIcon = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
    ...
    <path stroke="currentColor" strokeWidth="2.5" d="M3 3l18 18" />
  </svg>
);
const DeafenOnIcon = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
...
const PeerVideo = ({ peerId, username, streamData, isLocal, isMuted, isDeafened, isScreenSharing }) => {
  ...
  // Use hasVideoTrack to guarantee that remote screen shares expand fully
  const isActuallyScreenSharing = isScreenSharing || hasVideoTrack;

  const borderStyle = isActuallyScreenSharing
  ...
  return (
    <div className={`bg-[#1e1f22] rounded-xl overflow-hidden flex flex-col relative group border-2 ${borderStyle} transition-all duration-300
      ${isActuallyScreenSharing ? 'w-full origin-top' : 'flex-1 basis-[220px] max-w-[400px]'}
    `}
      style={{ minHeight: isActuallyScreenSharing ? '55vh' : '160px' }}
    >
      {/* Video o Placeholder */}
      {hasVideo ? (
        <video
